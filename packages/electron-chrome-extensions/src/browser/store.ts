...
export class ExtensionStore extends EventEmitter {
  ...
  async createTab(details: chrome.tabs.CreateProperties) {
    ...
    if (!details.windowId) {
      ...
      // If lastFocusedWindowId is still undefined, try to find any available window
      if (!details.windowId && this.windows.size > 0) {
        const firstWindow = Array.from(this.windows)[0]
        if (firstWindow && !firstWindow.isDestroyed()) {
          details.windowId = firstWindow.id
          this.lastFocusedWindowId = firstWindow.id
        }
      }
    }
