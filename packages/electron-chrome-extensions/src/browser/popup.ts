...
export class PopupView extends EventEmitter {
  ...
  private async load(url: string): Promise<void> {
    ...
    d(`loading popup URL: ${url}`)

    try {
      await win.webContents.loadURL(url)
      d(`successfully loaded popup URL: ${url}`)
    } catch (e) {
      d(`failed to load popup URL: ${url}`, e)
      console.error(`Failed to load popup URL: ${url}`, e)

      // If loading fails, destroy the popup
      this.destroy()
      return
    }
