...
class TabbedBrowserWindow {
  constructor(options) {
    ...
    // Window will be added to extensions store automatically via addTab
...
class Browser {
  ...
  async init() {
    ...
    this.extensions = new ElectronChromeExtensions({
      ...
      createTab: async (details) => {
        ...
        console.log('createTab called with details:', details)
        console.log(
          'Available windows:',
          this.windows.map((w) => ({ id: w.id, destroyed: w.window.isDestroyed() })),
        )

        // Check if this is an extension URL that should open in a separate window
        // Only create separate windows for specific extension pages (like onboarding), not for popups
        if (
          details.url &&
          details.url.startsWith('chrome-extension://') &&
          (details.url.includes('onboarding.html') || details.url.includes('options.html'))
        ) {
          console.log('Creating separate window for extension URL:', details.url)

          // Create a new standalone window for extension pages
          ...
          extensionWindow.setMenu(null)

          // Add the window's webContents as a tab to the extensions system
          this.extensions.addTab(extensionWindow.webContents, extensionWindow)
          ...
          if (process.env.SHELL_DEBUG) {
            extensionWindow.webContents.openDevTools({ mode: 'detach' })
          }

          console.log(`Extension window created for URL: ${details.url}`)
        ...
        if (!win) {
          // Try to use the first available window if windowId is not found
          const fallbackWin = this.windows.find((w) => !w.window.isDestroyed())
          if (fallbackWin) {
            console.log(`Using fallback window ${fallbackWin.id} instead of ${details.windowId}`)
            const tab = fallbackWin.tabs.create()
            if (details.url) tab.loadURL(details.url)
            if (typeof details.active === 'boolean' ? details.active : true)
              fallbackWin.tabs.select(tab.id)
            return [tab.webContents, tab.window]
          }
          throw new Error(
            `Unable to find windowId=${details.windowId}. Available windows: ${this.windows.map((w) => w.id).join(', ')}`,
          )
        }
    ...
    // this.createInitialWindow()  // Отключено: не создаем пустой браузер

    // Create standalone Phantom Wallet window
  ...
  createPhantomWindow() {
    ...
    // Create a standalone window for Phantom Wallet
    ...
    // Add the window's webContents as a tab to the extensions system
    // This will automatically add the window to the extensions store
    this.extensions.addTab(phantomWindow.webContents, phantomWindow)

    // Load Phantom popup.html
    const phantomPopupUrl = `chrome-extension://${phantomExtension.id}/popup.html`
    phantomWindow.loadURL(phantomPopupUrl)

    // Disable menu for Phantom window
    phantomWindow.setMenu(null)

    // Optional: Open DevTools for debugging
    if (process.env.SHELL_DEBUG) {
      phantomWindow.webContents.openDevTools({ mode: 'detach' })
    }

    console.log(`Phantom Wallet window created with URL: ${phantomPopupUrl}`)
